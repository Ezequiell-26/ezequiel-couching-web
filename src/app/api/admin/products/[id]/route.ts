...
const updateSchema = z
  .object({
    ...
    category: z.enum(["guia", "programa", "plantilla"]).optional(),
...
/** PUT /api/admin/products/[id] — edita un producto (parcial). */
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const existing = await db.product.findUnique({ where: { id: numId } });
    if (!existing) {
      return NextResponse.json({ ok: false, error: "Producto no encontrado." }, { status: 404 });
    }

    const product = await db.product.update({ where: { id: numId }, data: parsed.data });
...
/** DELETE /api/admin/products/[id] — borrado suave (active:false), nunca pérdida de histórico. */
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await db.product.update({ where: { id: numId }, data: { active: false } });
